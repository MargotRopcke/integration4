...
const ASSET = {
  thumbsUp:   "/assets/icons/hand-up.svg",
  thumbsDown: "/assets/icons/hand-down.svg",
  stopHand:   "/assets/icons/hand-undo.svg",
};
...
const INLINE = {
  thumbsUp: {
    ...
    sw: 11.5541, fillColor: "#B5E28D", strokeColor: "#5aad1e",
  },
  thumbsDown: {
    viewBox: "0 0 229 232", w: 229, h: 232,
    ...
    sw: 11.5541, fillColor: "#DA9B97", strokeColor: "#E03C31",
  },
  stopHand: {
    ...
    sw: 10, fillColor: "#FAECB3", strokeColor: "#FFCD00",
  },
};

export function ThumbUpSVG({ size = 60 }) {
  return <img src={ASSET.thumbsUp} width={size} height={size} alt="" aria-hidden="true" style={{ display: "block" }} />;
}
export function ThumbDownSVG({ size = 60 }) {
  return <img src={ASSET.thumbsDown} width={size} height={size} alt="" aria-hidden="true" style={{ display: "block" }} />;
}
export function StopHandSVG({ size = 60 }) {
  return <img src={ASSET.stopHand} width={size} height={size} alt="" aria-hidden="true" style={{ display: "block" }} />;
}
export function GestureSVG({ gesture, size = 60 }) {
  const src = ASSET[gesture];
  return src ? <img src={src} width={size} height={size} alt="" aria-hidden="true" style={{ display: "block" }} /> : null;
}

export function GestureProgressIcon({ gesture, progress = 0, size = 80, fillColor, baseColor }) {
  const uid = useId();
  const g = INLINE[gesture];
  if (!g) return null;
  ...
  return (
    <div
      className={`gesture-progress-icon${done ? " gesture-progress-icon--done" : ""}`}
      style={{ display: "inline-flex", alignItems: "center", justifyContent: "center" }}
      aria-hidden="true"
    >
      <svg width={size * aspect} height={size} viewBox={g.viewBox} fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <clipPath id={`gpi-${uid}`}>
            <rect x="0" y={clipTop} width={g.w} height={g.h} />
          </clipPath>
        </defs>
        <path d={g.path} fill={g.fillColor} fillOpacity={0.15} stroke={g.strokeColor} strokeOpacity={0.25} strokeWidth={g.sw} strokeLinecap="round" strokeLinejoin="round" />
        <path d={g.path} fill={g.fillColor} fillOpacity={0.75} stroke={g.strokeColor} strokeOpacity={1} strokeWidth={g.sw} strokeLinecap="round" strokeLinejoin="round" clipPath={`url(#gpi-${uid})`} />
      </svg>
    </div>
  );
}
